...
export class BooknowComponent {
  ...
  constructor(private router: Router, public nameofthehotel: HotelnameService, public searchbasedhotelname: SearchbasedhotelnameService, private formBuilder: FormBuilder) { }
  ...
  ngOnInit(): void {
    ...
    this.filteredvalues = hooteldetails;
    this.hotellist = hooteldetails;
  }
  isfilled() {
    try {
      if (this.location.length == 0 || this.count == 0 || this.from == null || this.to == null) {
        ...
        let filter = this.hotellist.filter(item => item.place.toLocaleLowerCase().includes(placename.toLocaleLowerCase()) && item.date == this.from)
        if (filter.length === 0) {
